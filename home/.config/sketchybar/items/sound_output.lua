...
local function current_output_device()
  return utils.capture(utils.shell_quote(SA_BIN) .. " -t output -c 2>/dev/null")
end

local function output_devices()
  return parse_lines(utils.capture(utils.shell_quote(SA_BIN) .. " -t output -a 2>/dev/null"))
end
...
local function set_output_device(name)
  ...
  os.execute(utils.shell_quote(SA_BIN) .. " -t output -s " .. utils.shell_quote(name) .. " >/dev/null 2>&1")
end
...
local function rebuild_popup()
  ...
  for i = 1, math.min(#devices, POPUP_SLOTS) do
    ...
    row_items[i]:set({
      ...
      click_script = utils.shell_quote(SA_BIN) .. " -t output -s " .. utils.shell_quote(dev)
