...
VOLUME_COLOR_ENV="VOLUME_MUTED_COLOR=$BASE04 VOLUME_ON_COLOR=$BASE0A VOLUME_LABEL_COLOR=$FOREGROUND"
...
sketchybar \
  ...
  --set "$VOLUME_ITEM" \
    ...
    script="$VOLUME_COLOR_ENV $PLUGIN_DIR/volume.lua --refresh" \
    update_freq=0 \
...
for i in 1 2 3 4 5 6 7 8; do
  ...
  sketchybar \
    ...
    --set "$name" \
      ...
      background.drawing=off \
      click_script="$VOLUME_COLOR_ENV $PLUGIN_DIR/volume.lua --select __EMPTY__"
done
